...
def plot_full_marginal_distributions(joint, alpha=0):
    """
    Five-panel bar chart: four symptom marginals plus full condition marginal.
    """
    ...
    for idx, name in enumerate(names):
        qv = [-1]*5
        qv[idx] = -2
        dist = inference_by_enumeration(joint, qv)
        ax = axes[idx]
        ...
        if idx < 4:
            ...
            ax.set_title("Condition Marginal")
            ax.set_xlabel("Condition Code")
            ax.tick_params(axis='x', rotation=90, labelsize=6)

    plt.suptitle(f"Marginal Distributions (α={alpha})")
...
def plot_calibration_curve(X_train, y_train, X_test, y_test, cls):
    ...
    probas = np.array([
        model.naive_bayes_inference({0:x[0],1:x[1],2:x[2],3:x[3]})
        for x in X_test
    ])
...
def main():
    ...
    # 1b. full marginals (including condition)
